...
Mat LineDetector::filter_only_yellow_white(Mat image) {
...
Mat LineDetector::crop_region_of_interest(Mat image) {
...
Mat LineDetector::draw_lines(Mat image, vector<Vec4i> lines) {
...
Mat LineDetector::detect_edges(Mat image) {
...
//i think this works have not checked
int** detected_matrix(Mat image){
  int** ret;
  Size S = image.size();
  int height = S.rows;
  int width = S.cols;
  ...
  for(int i = 0; i < height; i++){
    ret[i] = new int[width];
    //remember to clean this after using
